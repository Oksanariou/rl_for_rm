...
if __name__ == '__main__':
    ...
    for configuration_name in configuration_names:
        ...
        for dr_idx in range(len(demand_ratios)):
            ...
            experience_name = Path("../Results/"+configuration_name+"/"+str(demand_ratios[dr_idx]))
            list_of_rewards, mean_revenues1, mean_revenues2, mean_bookings, mean_bookings1, mean_bookings2, mean_prices_proposed1, mean_prices_proposed2 = env.collect_list_of_mean_revenues_and_bookings(experience_name)
            difference_to_true_revenue = ((mean_revenues1[training_idx] + mean_revenues2[training_idx])/(true_revenue1 + true_revenue2))*100
            ...
            for reward in list_of_rewards:
                list_final_revenues[dr_idx].append(((reward[:,0][training_idx] + reward[:,1][training_idx])/(true_revenue1 + true_revenue2))*100)
    ...
    # plt.savefig("../Results/comparison_demand_ratio_0.8.png")
    ...
    plt.ylabel("Relative revenue")
    # axes.set_ylim([0, 265])
    plt.savefig('../Results/multiagent_strategies_as_function_of_demand_ratios_training_'+str(training_idx)+'.png')
    ### UNCOMMENT
