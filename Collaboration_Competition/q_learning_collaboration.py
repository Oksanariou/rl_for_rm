...
def q_learning_collaboration3D(global_env, individual_env1, individual_env2, alpha, alpha_min, alpha_decay, gamma,
                               nb_episodes, epsilon,
    ...
    for episode in range(nb_episodes):
        ...
        while not (done1 and done2):
            ...
            Q1[state, action_idx1] = alpha * new_value1 + (1 - alpha) * old_value1

            old_value2 = Q2[state, action_idx2]
            new_value2 = (reward2 + gamma * np.max(Q2[next_state]))
            Q2[state, action_idx2] = alpha * new_value2 + (1 - alpha) * old_value2
    ...
    return [Q1, Q2], [revenue1, revenue2], episodes


def q_learning_collaboration2D(global_env, individual_env1, individual_env2, alpha, alpha_min, alpha_decay, gamma,
                               nb_episodes, epsilon,
                               epsilon_min, epsilon_decay, fully_collaborative):
    # Initialize the Q-table with zeros
    ...
    for episode in range(nb_episodes):
        ...
        while not (done1 and done2):
            ...
            Q1[state1, action_idx1] = alpha * new_value1 + (1 - alpha) * old_value1

            old_value2 = Q2[state2, action_idx2]
            new_value2 = (reward2 + gamma * np.max(Q2[next_state2]))
            Q2[state2, action_idx2] = alpha * new_value2 + (1 - alpha) * old_value2
