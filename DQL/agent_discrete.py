...
def DQNAgent_discrete_builder(env, parameters_dict):
    return DQNAgent_discrete(env, gamma=parameters_dict["gamma"], epsilon=parameters_dict["epsilon"],
...
class DQNAgent_discrete:
    ...
    def remember(self, state, action_idx, reward, next_state, done):
        ...
        next_state = to_categorical(next_state, self.input_size)
    ...
    def compute_state_weights(self):
        ...
        compute_weight = lambda s: 1 + max(1. * self.env.to_coordinate(s)[0] / self.env.T, 1. * self.env.to_coordinate(s)[1] / self.env.C)
        state_weights = [(s, compute_weight(s)) for s in range(self.env.nS)]

        return dict(state_weights)

    def compute_sample_weight(self, states):
        if self.state_weights is not None:
            return np.asarray([self.state_weights[state] for state in states])
        else:
            return None

    def compute_q_table(self, target=False):

        states = [s for s in range(self.env.nS)]
        states = [to_categorical(state, self.input_size) for state in states]

        model = self.target_model if target else self.model

        Q_table = model.predict(states).reshape(self.env.T, self.env.C, self.env.action_space.n)
